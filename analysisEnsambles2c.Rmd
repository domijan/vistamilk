...
```{r echo = FALSE}
load("analysisEnsambles2c2021.Rdata")

RMSEFULLDF <- matrix(0, 1000*14, 3) %>%as.data.frame()%>% mutate(trait = rep(names(ys),each = 1000))
# glimpse(RMSEFULLDF)
...
data.nest1.eff %>% ggplot( aes(y = fit, x = Trait, group = Algorithm, colour = Algorithm)) + geom_pointrange(aes(ymin = lower,
    ymax = upper)) + scale_y_continuous("RMSE") + theme(axis.text.x = element_text(angle = 90))+geom_line(aes(alpha = nonneg), show.legend = FALSE) +ylim(0.5,1.4)
```
...
data.nest1.eff %>% ggplot( aes(y = fit, x = Algorithm)) + geom_pointrange(aes(ymin = lower,
    ymax = upper)) + scale_y_continuous("RMSE") + theme(axis.text.x = element_text(angle = 90)) + geom_point(size = 3, pch = 1) +ylim(0.8,0.95)
# 
# library(sjPlot)
...
for(yind in 1:14){
  ...
  LMcoef <- matrix(0,50, 15)
  for (i in 1:50)LMcoef[i,] <- coef(nonneg.fit.SG[[i]][[yind]])[2:16]
  colnames(LMcoef) <- c("lasso", "elastic net", "pca", "pls", "LM+15", "RF", "RF+VS", "Bart", "kPCA","RF+kPCA", "RF+kernel", "svm",  "ppr", "brnn", "gbm")
  ...
  axis(1, at = 1 : 15, labels = colnames(LMcoef),las = 2, cex.axis=0.7)
   ...
   LMcoef <- matrix(0,50, 15)
  for (i in 1:50)LMcoef[i,] <- coef(lasso.fit.SG[[i]][[yind]])[2:16]
  colnames(LMcoef) <- c("lasso", "elastic net", "pca", "pls", "LM+15", "RF", "RF+VS", "Bart", "kPCA","RF+kPCA", "RF+kernel", "svm",  "ppr", "brnn", "gbm")
