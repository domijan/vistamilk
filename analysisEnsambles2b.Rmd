---
title: "Analysis with Ensambles: full data (14 traits)"
output:
...
<<<<<<< HEAD:analysisEnsambles2b.Rmd
=======
* I don't have id for the test set data used in the competition. I could work this out, but didn't for this analysis. Instead random 69 samples were removed (for comparison with copetition data). 
>>>>>>> 7ab471bbfc54b09249cbdfa3a4412dde5768aea7:analysisEnsambles2.Rmd
...
flds <- createFolds(1:200, k = 10, list = TRUE, returnTrain = FALSE)
  ...
  lmfitSG <- replicate(14, vector(mode = "list", length = N))
  lmfitSG2<- replicate(14, vector(mode = "list", length = N))
  rf.fitSG <- replicate(14, vector(mode = "list", length = N)) 
  lmfitSG3 <- replicate(14, vector(mode = "list", length = N)) 
for(yind in 1:14){
dat <- cbind(ys_tr[,yind], tr)
  ...
  for (i in 1:N){
    ...
    # plot(  lmfitSG[[i]][[yind]] )
    # plot(coef(lmfitSG2[[i]][[yind]]), coef(lmfitSG3[[i]][[yind]]))
      # sum(coef(lmfitSG3[[i]][[yind]]))
    
      # pred.stack <- as.matrix( predictions.tv) %*% as.matrix(lmfitSG[[i]][[yind]]$coef)
 ...
 # pred.stack2 <- predict(lmfitSG2[[i]][[yind]], predictions.te)
  ...
  #  pred.stack3 <- predict(lmfitSG3[[i]][[yind]], predictions.te)
    ...
    # plot(dat.tr.full[, 1], as.matrix( predictions.tv) %*% as.matrix(lmfitSG[[i]][[yind]]$coef))
    #   abline(a = 0, b = 1)  
      
      #   plot(dat.tr.full[, 1], predict(lmfitSG2[[i]][[yind]], predictions.tv))
      # abline(a = 0, b = 1)  
      
      #    plot(dat.tr.full[, 1], predict(rf.fitSG[[i]][[yind]], data = cbind(predictions.tv, dat.tr.full[, -1]))$prediction)
      # abline(a = 0, b = 1)  
      
    # plot(coef(lmfitSG3[[i]][[yind]]), coef(lmfitSG2[[i]][[yind]])
      # plot(dat.tr.full[, 1], predict(lmfitSG3[[i]][[yind]], predictions.tv))
      # abline(a = 0, b = 1)
    ##-----------------------------------------------------------
    j <- 1
...
save.image("analysisEnsambles2b.Rdata")
...
```{r echo = FALSE}
load("analysisEnsambles2b.Rdata")
