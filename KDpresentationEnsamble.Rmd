...
###########################################
 ...
 pca.pred.poor.tv <- model.matrix(V1 ~. , data = dat.tv[, c(1, nonz+1)]) %*% as.matrix(lmfit$coef)
  ...
  Ktest2 <- kernelMatrix(kfunc, as.matrix(dat.te[,2:1061]), as.matrix(dat.tr[,2:1061]))
   Kv2 <- kernelMatrix(kfunc, as.matrix(dat.tv[,2:1061]), as.matrix(dat.tr[,2:1061]))
 
  kpcKern2 <- kPCA(Ktrain2)


  KPCpred3 <- predict(kpcKern2, Ktest2)
  KPCpredtv <- predict(kpcKern2, Kv2)
  ...
  pca.pred.poor.te <- cbind(rep(1,nrow(KPCpred3)), KPCpred3[ , 1 : 6]) %*% as.matrix(lmfit$coef)
    ...
    pca.pred.poor.tv <- cbind(rep(1,nrow(KPCpredtv)), KPCpredtv[ , 1 : 6]) %*% as.matrix(lmfit$coef)
  ...
  RMSE[i,j] <- calcRMSE(dat.te[,1], pred.rf$predictions)
  predictions[,j] <- pred.rf$predictions
  predictions.tv[,j] <- predict(rf.fit , data = KPCpredtv[ , 1 : 6])$predictions
  j <- j+1
  ...
  RMSE[i,j] <- calcRMSE(dat.te[,1], pred.rf$predictions)
  predictions[,j] <- pred.rf$predictions
  predictions.tv[,j] <- predict(rf.fit , data = Kv2)$predictions
  j <- j+1
...
# image(t(Coefs1))
# image(t(Coefs2))
# plot(dat.te[,1],rowMeans(predictions[, 1:13]),  col = as.numeric(dat.te$class)+1, xlab = "kappa_casein validation set: one random split", ylab = "predicted",  main = "ensamble")
# abline(0,1)
...
# dim(dat.te)
# pairs(rbind(cbind(dat.tv[, 1],predictions.tv[, 1:13]), cbind(dat.te[, 1],predictions[, 1:13])), col = c(rep(1, 130), rep(2, 139)))
# 
...
# 
#     lasso.fit <- glmnet(predictions,dat.te[,1],alpha=1, lambda = 0, lower.limits = 0, intercept = FALSE) 
# 
# coef( lasso.fit)
# 
# library(condvis2)
