...
colnames(RMSE) <- c("lasso", "lm+pca", "pls3", "pls4","pls5",  "lm+15", "rf", "rf+vs", "bart", "lm+kpca","rf+kpca", "rf+kernel", "svm",  "ensambleMA",  "ensambleStack",  "ensambleStack2", "ensambleStack3")
...
# ====replace below]
RMSE <- matrix(0, N, 17)
predictions <- matrix(0,nrow(dat)-260, 13)
predictions.tv <- matrix(0,130, 13)
...
###########################################
 ...
 pca.pred.poor.tv <- model.matrix(V1 ~. , data = dat.tv[, c(1, nonz+1)]) %*% as.matrix(lmfit$coef)
  ...
  RMSE[i,j] <-  calcRMSE(dat.te[,1],   rowMeans(predictions[, 1:13]))
   ...
   colnames(predictions.tv) <- c("lasso", "lm+pca", "pls3", "pls4","pls5",  "lm+15", "rf", "rf+vs", "bart", "lm+kpca","rf+kpca", "rf+kernel", "svm")
    colnames(predictions) <- c("lasso", "lm+pca", "pls3", "pls4","pls5",  "lm+15", "rf", "rf+vs", "bart", "lm+kpca","rf+kpca", "rf+kernel", "svm")
  lmfit<- lm(dat.tv[, 1]~predictions.tv[, 1:13] - 1)
# summary(  lmfit)
  
  pred.stack <- as.matrix( predictions[, 1:13]) %*% as.matrix(lmfit$coef)
  # pred.stack <- as.matrix(cbind(rep(1, nrow(x.te)), predictions[, 1:13])) %*% as.matrix(lmfit$coef)
   lmfit2<- glmnet(predictions.tv,dat.tv[,1],alpha=1, lambda = 0, lower.limits = 0, intercept = FALSE) 
# summary(  lmfit)
  
    pred.stack2 <- predict(lmfit2, predictions)
    
    rf.fitSG <- ranger(y = dat.tv[, 1], x = cbind( predictions.tv, dat.tv[, -c(1,1062)]), importance = "impurity")

  pred.rf <- predict(rf.fitSG , data = cbind(predictions, dat.te[, -c(1,1062)]))

# summary(  lmfit)
  
    pred.stack3 <- predict(rf.fitSG , data = cbind(predictions, dat.te[, -c(1,1062)]))$prediction
   RMSE[i,j+1] <- calcRMSE(dat.te[,1],  pred.stack)
    RMSE[i,j+2] <- calcRMSE(dat.te[,1],  pred.stack2)
      RMSE[i,j+3] <- calcRMSE(dat.te[,1],  pred.stack3)
