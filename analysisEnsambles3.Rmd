---
title: "Analysis with Ensambles: competition data"
output:
...
* All columns are used (1060 features).
* The transmittance values are not transformed to absorbance values.
* Only have responses for the training set (not test set 69 onservation) 
...
* 10 fold cross - validation on each of the 50 training sets to train stack ensambles. 


## Add more ensamble classifiers

* ensambleMA: algorithms are trained on the full training set, then use post-hoc approach of averaging over the predictions of all the algorithms of the test set. The averaged predictions are the ensambleMA predictions for the test set. Done over 50 random splits.

**Stacking approaches:** Use 10-fold cross - validation of the training sets to train all algorithms.
Fit a regression model with trait values in the training set as response and predictions from the holdout samples of the training set as predictors. Use the resulting coefficients to combine test set predictions.  Done over 50 random splits.

* ensambleLM:  regression to combine predictions (using lm)
* ensambleNonNeg: regression with restriction to non-negative coefficients (using glmnet)
* ensambleRF: RF to combine predictions plus all the predictors (using ranger).
...
flds <- createFolds(1:200, k = 10, list = TRUE, returnTrain = FALSE)
  ...
  predictions.te3 <- array(0,c(nrow(tr)-200,14, N))
  predictions.tv3 <- array(0,c(200,14, N))
for(yind in 1:3){
...
save.image("analysisEnsambles3.Rdata")

```

Note placing constrains for non-negative coefficients sometimes causes glmnet fit to fail to converge.

```{r echo = FALSE}

load("analysisEnsambles3.Rdata")
...
=======
  yind
>>>>>>> 7ab471bbfc54b09249cbdfa3a4412dde5768aea7
    if (yind ==2) dat <- cbind(log(ys_tr[,yind]), tr)else dat <- cbind(ys_tr[,yind], tr)
...
>>>>>>> 7ab471bbfc54b09249cbdfa3a4412dde5768aea7
  ...
  matplot(t(LMcoef), type = "l", lty = 1, xaxt = "n", col = 9, ylab = "Estimated coefficients", main = "ensemble LM")
  abline(h  = 0)
  ...
  matplot(t(LMcoef), type = "l", lty = 1, xaxt = "n", col = 9, ylab = "Estimated coefficients", main = "ensemble Non Neg")
  abline(h  = 0)
  axis(1, at = 1 : 14, labels = colnames(LMcoef),las = 2, cex.axis=0.7)
  ...
  matplot(t(LMcoef), type = "l", lty = 1, xaxt = "n", col = 9, ylab = "Estimated coefficients", main = "ensemble RF")
  abline(h  = 0)
  axis(1, at = 1 : 14, labels = colnames(LMcoef),las = 2, cex.axis=0.7)
...
for(i in 1:50) print(sum(lmfitSG2[[i]][[yind]]$beta)-1)
```
