...
for(yind in 1:14){
  ...
  RMSE <- matrix(0, N, 25)
  predictions.te <- matrix(0,nrow(dat)-200, 24)
  predictions.tv <- matrix(0,200, 23)
  ...
  for (i in 1:N){
    ...
    pls.fit <- plsr(V1 ~., data=dat.tr, scale= TRUE, validation = "CV")
     ...
     for (z in 3:15){
     pls.pred <- predict(pls.fit, dat.te[,-1], ncomp = z)
  ...
  j <- j+ 1 
    ...
    Ktest <- kernelMatrix(kfunc, as.matrix(dat.te[,2:ncol(dat.tr)]), as.matrix(dat.tr[,2:ncol(dat.tr)]))
    kpcTrain <- kPCA(Ktrain)
    ...
    RMSE[i,j] <-  calcRMSE(dat.te[,1],   rowMeans(predictions.te[, 1:24]))
  }
  
  colnames(RMSE) <- c("lasso", "elastic net", "pca", "pls3", "pls4","pls5", "pls6", "pls7","pls8",  "pls9", "pls10","pls11", "pls12", "pls13","pls14", "pls15", "LM+15", "RF", "RF+VS", "Bart", "kPCA","RF+kPCA", "RF+kernel", "svm", "ensamble MA")
  RMSEFULL[[yind]] <- RMSE
  
  RMSE1 <- RMSE  %>%as.data.frame()%>% mutate(split = as.factor(1:N))%>% pivot_longer(1:25,"ALGORITHM")
