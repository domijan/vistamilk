...
for (i in 1:N){
  ...
  pca.pred.poor.te <- as.matrix(cbind(rep(1, nrow(x.te)), x.te[,1:5])) %*% as.matrix(lm.fit$coef[1:6])
  ...
  pca.pred.poor.te <- x %*% as.matrix(lm.fit$coef)
 ...
 kpcKern2 <- kPCA(Ktrain2)
  ...
  pca.pred.poor.te <- cbind(rep(1,nrow(KPCpred3)), KPCpred3[ , 1 : 6]) %*% as.matrix(lm.fit$coef)
